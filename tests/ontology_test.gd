...
func run() -> void:
	...
	for person_id in [
		...
		var standard_portrait_texture := String(person.get("standard_portrait_texture", ""))
		...
		assert(ResourceLoader.exists(standard_portrait_texture, "Texture2D"), "standard portrait texture must import for %s: %s" % [person_id, standard_portrait_texture])
