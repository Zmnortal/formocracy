...
func run() -> void:
	...
	var module := WorkbenchNpcPerformanceModule.new(scene)
	...
	var target := module._fully_offscreen_exit_position(actor)
	...
	assert(await _wait_until(func() -> bool: return module.state == "IDLE", 2.5))
...
func _wait_until(predicate: Callable, timeout_seconds: float) -> bool:
	...
	while elapsed < timeout_seconds:
		if WorkdayContext.to_bool(predicate.call()):
			return true
		await process_frame
		elapsed += 1.0 / 60.0
	return WorkdayContext.to_bool(predicate.call())
