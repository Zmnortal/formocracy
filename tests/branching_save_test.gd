...
func run() -> void:
	...
	assert(state.save_system.create_initial_checkpoint(), "opening must create the initial checkpoint")
	...
	nodes = state.save_system.get_checkpoint_nodes()
	...
	assert(state.save_system.load_checkpoint(day_one_id), "historical day one must load")
	...
	assert(state.save_system.delete_checkpoint(delete_id), "one branch must be deletable")
	nodes = state.save_system.get_checkpoint_nodes()
	assert(find_children(nodes, day_one_id, 2).size() == 1, "deleting one branch must preserve its sibling")
	assert(not state.save_system.delete_checkpoint(root_id), "the beginning root must never be deletable")
