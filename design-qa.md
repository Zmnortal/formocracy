...
- Image quality and asset fidelity: the desk is a dedicated raster environment asset. Machine, rail, foreground mask, and lights use approved project textures. The validation sleeve is a deterministic color-filter derivative of the original closed bureau envelope: dimensions remain `340 × 520`, and an alpha-channel comparison reports `0` changed pixels, so the silhouette, flap, folds, and string closure are exactly preserved. No label window, stripe, or generated decoration remains.
...
- A plain click does not select a numbered document.
- Pressing and dragging a document reveals a bounded waiting-zone target; releasing inside it stages the bag without starting the conveyor.
- Dragging a staged document back below the waiting zone returns it to its original numbered slot.
